...
internal class AtProtoSyncClient(context:Context){
 private val http=OkHttpClient(); private val jsonType="application/json; charset=utf-8".toMediaType(); private val syncUrl="https://fbtloeehynqobbwcndru.supabase.co/functions/v1/kelo-id-atproto-sync"; private val qrApproveUrl="https://kelo-id.eu/api/integrations/kelo-social/mobile-approve"
 ...
 suspend fun approveKeloSocialLoginQr(s:AtSession,challenge:String)=withContext(Dispatchers.IO){val body=JSONObject().put("challenge",challenge).put("pdsUrl",s.pdsUrl).toString().toRequestBody(jsonType);val r=Request.Builder().url(qrApproveUrl).post(body).header("Authorization","Bearer ${s.accessJwt}").header("Accept","application/json").build();http.newCall(r).execute().use{res->val raw=res.body?.string().orEmpty();if(!res.isSuccessful){val d=runCatching{JSONObject(raw)}.getOrNull();throw IllegalStateException(d?.optString("error").takeIf{!it.isNullOrBlank()}?:"Impossible d’autoriser la connexion QR.")};return@withContext}}
